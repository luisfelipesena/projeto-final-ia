...
\begin{frame}{Sensores}
    ...
    \begin{tikzpicture}[scale=0.85]
        % Robot body (top view)
        \draw[fill=gray!20, thick, rounded corners=5pt] (-1.5,-2.2) rectangle (1.5,2.2);
        \node[font=\small\bfseries] at (0,-0.5) {YouBot};

        % LIDAR rays (180 degrees from front)
        \foreach \a in {-85,-70,-55,-40,-25,-10,5,20,35,50,65,80} {
            \draw[red!60, ->, line width=1pt] (0,2.2) -- ++(\a+90:3);
        }
        \node[red!80, font=\bfseries] at (0,5.8) {LIDAR 180°};
        \node[red!60, font=\scriptsize] at (0,5.3) {5m range};

        % Camera FOV cone
        \draw[blue!60, fill=blue!15, line width=1.5pt] (0,2.2) -- ++(55:2.5) arc (55:125:2.5) -- cycle;
        \node[blue!80, font=\bfseries] at (0,4) {Câmera RGB};

        % Distance sensors (6 dots)
        \foreach \x in {-0.8,0,0.8} {
            \fill[green!70!black] (\x,2.2) circle (0.12);
            \fill[green!70!black] (\x,-2.2) circle (0.12);
        }
        \node[green!70!black, font=\bfseries] at (0,-3) {6 Dist. Sensors};

        % Legend box (right side)
        \node[draw, rounded corners, fill=yellow!10, font=\scriptsize, text width=4cm, align=left] at (5.5,0) {
            \textbf{Função:}\\[0.3em]
            \textcolor{red!70}{$\bullet$ LIDAR} $\rightarrow$ Obstáculos\\[0.2em]
            \textcolor{blue!70}{$\bullet$ Câmera} $\rightarrow$ Cubos + CNN\\[0.2em]
            \textcolor{green!60!black}{$\bullet$ Dist.} $\rightarrow$ Colisão
        };
    \end{tikzpicture}
\end{frame}
...
\begin{frame}{Máquina de Estados}
    ...
    \begin{tikzpicture}[
        scale=1.1,
        state/.style={draw, rectangle, rounded corners, minimum width=2.2cm, minimum height=1cm, thick, font=\small\bfseries},
        arrow/.style={->, thick, >=stealth}
    ]
        % States (circular layout)
        \node[state, fill=yellow!40] (search) at (0,2) {SEARCH};
        \node[state, fill=orange!40] (approach) at (4,2) {APPROACH};
        \node[state, fill=green!40] (grasp) at (6,0) {GRASP};
        \node[state, fill=purple!40] (tobox) at (4,-2) {TO\_BOX};
        \node[state, fill=red!40] (drop) at (0,-2) {DROP};

        % Transitions with icons
        \draw[arrow, blue!70, line width=1.5pt] (search) -- node[above, font=\tiny] {detectado} (approach);
        \draw[arrow, blue!70, line width=1.5pt] (approach) -- node[right, font=\tiny, pos=0.3] {perto} (grasp);
        ...
        \draw[arrow, blue!70, line width=1.5pt] (drop) -- node[left, font=\tiny] {$\times$15} (search);

        % Self-loop (lost)
        \draw[arrow, red!60, dashed] (approach.north) .. controls (2,3.5) and (-2,3.5) .. node[above, font=\tiny] {perdido} (search.north);
    \end{tikzpicture}
\end{frame}
